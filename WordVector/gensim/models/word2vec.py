...
class Word2Vec(utils.SaveLoad):
    ...
    def reset_weights_topic(self, topic_number):
		"""
		reset_weights for topics 
		"""
        random.seed(self.seed)
        self.syn0_topic = empty((topic_number, self.layer1_size), dtype=REAL) #W
    ...
    def train_topic(self, topic_number, sentences, total_words=None, word_count=0, chunksize=100):
        ...
        def worker_train():
            ...
            while True:
                ...
                # how many words did we train on? out-of-vocabulary (unknown) words do not count
                if self.sg:
                    job_words = sum(train_sentence_topic(self, sentence, alpha, work) for sentence in job)#syn0 ->input,syn1->W
                else:
                    job_words = sum(train_sentence_cbow(self, sentence, alpha, work, neu1) for sentence in job)
                with lock:
                    word_count[0] += job_words
