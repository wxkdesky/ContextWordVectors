...
int model::init_inf() {
    // estimating the model from a previously estimated one
    int m, n, w, k;
	//K is the topic number,M is the document number,V is the vocabulary number, of the trained model
    p = new double[K];
    ...
    nw = new int*[V];//V is the vocabulary of the trained model
    for (w = 0; w < V; w++) {
        nw[w] = new int[K];
        for (k = 0; k < K; k++) {
    	    nw[w][k] = 0;
    ...
    newM = pnewdata->M;
    newV = pnewdata->V;//new vocabulary in the inferred data
